...
// All the following proofs have been generated through the use of <https://github.com/panva/dpop>
...
func TestParse_ExpiredProof(t *testing.T) {
    ...
    dpopProofClaims := createDpopProofClaims(&httpUrl)
    ...
    if err != nil {
        AssertJoinedError(t, err, jwt.ErrTokenExpired)
    }
    if proof != nil {
        t.Errorf("Expected nil token")
    }
...
func TestParse_ProofSignedTooFarIntoFuture(t *testing.T) {
    ...
    if err != nil {
        AssertJoinedError(t, err, jwt.ErrTokenUsedBeforeIssued)
    }
    if proof != nil {
        t.Errorf("Expected nil token")
    }
...
func TestParse_ProofWithSkew(t *testing.T) {
    httpUrl := url.URL{
        Scheme: "https",
        Host:   "server.example.com",
        Path:   "/token",
    ...
    // Set all claims which are affected by skew
    dpopProofClaims := createDpopProofClaims(&httpUrl)
    dpopProofClaims.NotBefore = &jwt.NumericDate{
        Time: time.Now().Add(1 * time.Minute),
    }
    dpopProofClaims.IssuedAt = &jwt.NumericDate{
        Time: time.Now().Add(1 * time.Minute),
    }
    dpopProofClaims.ExpiresAt = &jwt.NumericDate{
...
func createSignedDpopProofTokenString(dpopProofClaims *dpop.ProofTokenClaims, t *testing.T) string {
    dpopToken := createEcdsaDpopProofToken(dpopProofClaims)
    boundTokenClaims := createBoundTokenClaims(dpopToken)
    boundToken := createBoundToken(boundTokenClaims, rsaPrivateKey)
    dpopProofClaims, ok := dpopToken.Claims.(*dpop.ProofTokenClaims)
    if !ok {
        t.Fatalf("failed to type assert claims")
    }
    dpopProofClaims.AccessTokenHash = createBase64Hash([]byte(boundToken))
    dpopProofToken, err := dpopToken.SignedString(ecdsaPrivateKey)
    if err != nil {
        t.Fatalf("Error creating DPoP Proof String: %s", err)
    }
    return dpopProofToken
}

func createDpopProofClaims(httpUrl *url.URL) *dpop.ProofTokenClaims {
...
func createBoundToken(claims jwt.Claims, privateKey *rsa.PrivateKey) string {
    token := jwt.NewWithClaims(jwt.SigningMethodRS256, claims)
    jwToken, err := token.SignedString(privateKey)
    if err != nil {
        panic(err)
    }
    return jwToken
...
func createBase64Hash(boundToken []byte) string {
    h := sha256.New()
    _, err := h.Write(boundToken)
    if err != nil {
        panic(err)
    }
...
func createBoundTokenClaims(dpopProofToken *jwt.Token) *dpop.BoundAccessTokenClaims {
    jwkHeaderJson, err := json.Marshal(dpopProofToken.Header["jwk"])
    if err != nil {
        panic(err)
    }
    base64sha := createBase64Hash(jwkHeaderJson)

    return &dpop.BoundAccessTokenClaims{
        RegisteredClaims: &jwt.RegisteredClaims{
            ExpiresAt: jwt.NewNumericDate(time.Now().Add(900e9)),
            IssuedAt:  jwt.NewNumericDate(time.Now()),
            Subject:   "test01",
            Issuer:    "Tokenizer",
            Audience:  jwt.ClaimStrings{"166ee9a2-2052-4979-af0f-c1f9cb37ad98"},
        },
        Confirmation: dpop.Confirmation{
            JWKThumbprint: base64sha,
        },
    }
}

func createEcdsaDpopProofToken(dpopProofClaims *dpop.ProofTokenClaims) *jwt.Token {
    ...
    jwk := &ecdsaJwk{
        Crv: ecdsaPrivateKey.PublicKey.Curve.Params().Name,
        Kty: "EC",
        X:   base64.RawURLEncoding.EncodeToString(ecdsaPrivateKey.PublicKey.X.Bytes()),
        Y:   base64.RawURLEncoding.EncodeToString(ecdsaPrivateKey.PublicKey.Y.Bytes()),
    }
